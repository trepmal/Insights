...
if ( $_GET['search'] ) {
	...
	// echo $_GET['search'] . '<br />';
	switch( $_GET['mode'] ) {
		case '6': die( search_google( $_GET['search'] ) );
		case '5': die( search_maps( $_GET['search'] ) );
		case '4': die( search_wiki( $_GET['search'] ) );
		case '3': die( search_videos( $_GET['search'] ) );
		case '2': die( search_images( $_GET['search'] ) );
		default:  die( search_posts( $_GET['search'] ) );
	}

} else {
	die('eh?');
}
function search_maps( ) { die( '###' ); }
...
function search_posts( $search ) {
	...
	if ( $posts )
		foreach ( $posts as $post ) {
			...
			$output .= '
	<p><a style="text-decoration:none;" href="'. get_permalink( $post->ID ) . '" target="_blank"><strong>' . $post->post_title . '</strong></a>
	<a href="'. get_edit_post_link( $post->ID ) .'"  ><img title="Edit '.$post->post_type.'"  src="'. plugins_url('/img/edit.png', __FILE__ ) .'" /></a>
	<img title="Insert link to selection" style="cursor:pointer;" onclick="insert_link(\''. get_permalink( $post->ID ) .'\');" src="'. plugins_url( '/img/link.png', __FILE__ ) .'" /></p>
	<p>' . get_excerpt( $post->post_content, 50 ) . '</p>';
		} //end foreach
	else
...
<br />';
		...
		// output image
		if ( $tag_images ) $output .= $tag_images;

		if ( $text_images ) $output .= $text_images;
...
function search_videos($keyword) {
	...
	foreach ( $index['MEDIA:PLAYER'] as $id ) {
		...
		$file   = $matches[3];
		// $out .= print_r( $index, true );
		$title  = $vals[ $index['MEDIA:TITLE'][ $count ] ]['value'];
		$desc   = isset( $vals[ $index['MEDIA:DESCRIPTION'][ $count ] ]['value'] ) ?
					$vals[ $index['MEDIA:DESCRIPTION'][ $count ] ]['value'] : '';
		$views  = $vals[ $index['YT:STATISTICS'][ $count ] ]['attributes']['VIEWCOUNT'];
		$rating = isset( $index['GD:RATING'][ $count ] ) ?
					$vals[ $index['GD:RATING'][ $count ] ]['attributes']['AVERAGE'] : '';

		// js: function show_video(ytfile, yttitle, ytdesc, ytviews, ytrating) {

		$out.= '<img style="cursor:pointer" src="http://img.youtube.com/vi/'.$file.'/default.jpg" width="130" height="97" hspace="2" vspace="2" border="0" title="'.str_replace('"', "", $title).'" 
		onclick="show_video(\'' . $file . '\',
			\'' . esc_js( $title ) . '\',
			\'' . esc_js( $title ) . '\',
			\'' . $views . '\',
			\'' . $rating . '\');">';

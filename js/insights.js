...
function show_video(ytfile, yttitle, ytdesc, ytviews, ytrating) {
	...
	var data='<h4>'+yttitle+'</h4><p><a href="http://www.youtube.com/watch?v='+ytfile+'">' + InsightsSettings.txt_youtubelink + '</a></p>'+
	'<p>'+ytdesc+'</p><p>' + InsightsSettings.txt_youtubeviews.replace('{views}', ytviews ) + '</p>'+
	'<p>' + InsightsSettings.txt_youtuberating.replace('{rating}', ytrating ) + '</p>';
	var button='<br /><p><input class="button" type="button" value="' + InsightsSettings.txt_youtubeadd + '" onclick="insert_video();" ></p>'+
	'<p>' + InsightsSettings.txt_youtubeoembed + '</p>';
	...
	function submit_me() {
			...
			$.getJSON('//en.wikipedia.org/w/api.php?action=query&list=search&srwhat=text&srlimit=10&srsearch='+escape(phrase)+'&format=json&callback=?',
				function( data ) {
					...
					if ( !data.query.search.length )
						output = InsightsSettings.txt_youtubeviews.replace('{query}', phrase );
					else
						$.each( data.query.search, function( i, item ) {
							output = output+'<p><a target="_blank" style="text-decoration:none;" href="'+ wikipediaUrl + item.title.replace(/ /g, "_")+'" ><strong>'+item.title+ '</strong></a> <img title="' + InsightsSettings.txt_insertlink + '" style="cursor:pointer;" onclick="insert_link(\''+wikipediaUrl + item.title.replace(/ /g, "_")+'\');" src="'+ InsightsSettings.insights_url +'/img/link.png" /></p>';
						});
				...
				function( data ) {
					...
					if ( !data.responseData.results.length )
						...
						$.each( data.responseData.results, function( i, item ) {
							output = output+'<p><a target="_blank" style="text-decoration:none;" href="'+item.url+'"><strong>'+ item.titleNoFormatting +'</strong></a> <img style="cursor:pointer;" title="' + InsightsSettings.txt_insertlink + '" onclick="insert_link(\''+item.url+'\');" src="'+InsightsSettings.insights_url+'/img/link.png" /><p>'+item.content+'</p></p>';
						});
			...
			$.getJSON("//ajax.googleapis.com/ajax/services/search/news?q="+escape(phrase)+"&v=1.0&scoring=d&rsz=large&callback=?",
				function( data ) {
					...
					if ( !data.responseData.results.length )
						...
						$.each( data.responseData.results, function( i, item ) {
							output = output+'<p><a target="_blank" style="text-decoration:none;" href="'+item.unescapedUrl+'"><strong>'+ item.titleNoFormatting+'</strong></a> <img style="cursor:pointer;" title="' + InsightsSettings.txt_insertlink + '" onclick="insert_link(\''+item.url+'\');" src="'+InsightsSettings.insights_url+'/img/link.png" /><br />'+item.publisher+', '+item.location+' on '+item.publishedDate +'<p>'+item.content+'</p></p>';
						});
			...
			$.getJSON("//ajax.googleapis.com/ajax/services/search/blogs?q="+escape(phrase)+"&v=1.0&scoring=d&rsz=large&callback=?",
				function( data ) {
					...
					if ( !data.responseData.results.length )
						...
						$.each( data.responseData.results, function( i, item ) {
							output = output+'<p><a target="_blank" style="text-decoration:none;" href="'+item.postUrl+'"><strong>'+ item.titleNoFormatting+'</strong></a> <img style="cursor:pointer;" title="' + InsightsSettings.txt_insertlink + '" onclick="insert_link(\''+item.postUrl+'\');" src="'+InsightsSettings.insights_url+'/img/link.png" /><br />'+item.blogUrl+'<p>'+item.content+'</p></p>';
						});
			...
			$.getJSON("//ajax.googleapis.com/ajax/services/search/books?q="+escape(phrase)+"&v=1.0&as_brr=1&rsz=large&callback=?",
				function( data ) {
					...
					if ( !data.responseData.results.length )
						...
						$.each( data.responseData.results, function( i, item ) {
							output = output+'<p><a target="_blank" style="text-decoration:none;" href="'+item.unescapedUrl+'"><strong>'+ item.titleNoFormatting+'</strong></a> <img style="cursor:pointer;" title="' + InsightsSettings.txt_insertlink + '" onclick="insert_link(\''+item.unescapedUrl+'\');" src="'+InsightsSettings.insights_url+'/img/link.png" /><p>'+ item.authors+', published '+item.publishedYear+', '+item.pageCount+' pages </p></p>';
						});
		...
		if ( cached ) {
			...
			$.ajax( InsightsSettings.ajaxurl, {
				...
				datatype: "string",
				error: function() {
					$('#insights-results').html( InsightsSettings.txt_ajaxerror );
				},
